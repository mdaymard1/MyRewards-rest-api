import {
  ...
  LoyaltyPromotion,
  Merchant,
  RetrieveMerchantResponse } from 'square';
import { decryptToken } from "./EncryptionService";

import dotenv from 'dotenv';

export const getMerchantInfo = async (merchantId: string, accessToken: string, callback: any) => {
  ...
  try {
    ...
    if (merchantResponse?.result?.merchant) {
      console.log("returning merchant for id: " + merchantResponse?.result?.merchant?.id);
...
export const getMainLoyaltyProgramFromMerchant = async (token: string, callback: any) => {
  ...
  const { catalogApi, loyaltyApi } = client;
    ...
    try {
      let loyaltyProgramResponse = await loyaltyApi.retrieveLoyaltyProgram("main");
      ...
      let promotionsResponse = await loyaltyApi.listLoyaltyPromotions(program.id!, "ACTIVE");
      ...
      let scheduledPromotionsResponse = await loyaltyApi.listLoyaltyPromotions(program.id!, "SCHEDULED");
      console.log("scheduledPromotionsResponse: " + scheduledPromotionsResponse?.result);
