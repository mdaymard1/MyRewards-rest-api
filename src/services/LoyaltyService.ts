...
export const getLoyaltyEnrollmentSettings = async (businessId: string) => {
...
export const notifyCustomersOfChanges = async (
  businessId: string,
  ...
  const changeTypeWhereClause =
    type == NotificationChangeType.Rewards
      ? "notificationPref.notifyOfRewardChanges = true"
      : type == NotificationChangeType.Promotions
  ...
  const customers = await CustomerNotificationPreference.createQueryBuilder(
    "notificationPref"
  )
  ...
  if (customers && customers.length > 0) {
    ...
    if (customerIds.length > 0) {
      ...
      sendNotifications(contents, customerIds, locationImage);
    }
  }
};
...
export const enrollCustomerInLoyalty = async (
  ...
  if (!loyaltyCustomerAccountId) {
    ...
    console.log(
      "no loyaltyCustomerAccountId cause loyalty probably exists. updating customer"
    );
  ...
  var merchCustomerId: string | null | undefined;
  if (appCustomerId) {
...
const updateExistingCustomer = async (
  ...
  let existingMerchantCustomerId = await lookupMerchantCustomerIdByPhoneNumber(
    token,
    phoneNumber
  );
  if (existingMerchantCustomerId) {
    ...
    if (appCustomerId) {
      await upsertMerchantCustomerAccount(
        token,
        existingMerchantCustomerId,
        appCustomerId,
        phoneNumber,
...
const insertCustomer = async (
  ...
  try {
    ...
    if (
      ...
      let existingCustomer = await Customer.createQueryBuilder("customer")
...
export const updateLoyaltyFromWebhook = async (
  ...
  if (webhookLoyaltyProgram.accrualRules && webhookLoyaltyProgram.terminology) {
    ...
    wasSuccessful = await cleanUpLoyaltyAndSaveChanges(
      loyalty!,
  ...
  if (
    ...
    const notificationContents =
      business?.businessName + " has made some changes to its loyalty program.";
...
const cleanUpLoyaltyAndSaveChanges = async (
  ...
  if (loyalty?.loyaltyAccruals) {
    await removeOldAccrualRules(loyalty.loyaltyAccruals, webhookLoyaltyProgram);
  }
  //Remove old reward tier rows
  if (loyalty?.loyaltyRewardTiers) {
    await removeOldRewardTiers(
      loyalty.loyaltyRewardTiers,
      webhookLoyaltyProgram
    );
  }
...
export const updateLoyaltyAccountFromWebhook = async (
  ...
  const pointAdjustment = await upsertCustomerFromWebhook(
    business.businessId,
  ...
  if (pointAdjustment && pointAdjustment > 0) {
    ...
    if (customer) {
      ...
      const pointLabel =
        pointAdjustment == 1 ? loyalty.terminologyOne : loyalty.terminologyMany;
      const contents =
        "You just earned " +
        pointAdjustment +
        " " +
...
export const updatePromotionsFromWebhook = async (
  ...
  if (promotion) {
    ...
    const existingLoyalty = await Loyalty.createQueryBuilder("loyalty")
      ...
      .getOne();
    if (existingLoyalty) {
      await createPromotion(webhookLoyaltyPrmotion, existingLoyalty.id);
      wasSuccessful = true;
    } else {
  ...
  if (
    ...
    const notificationContents =
      business?.businessName + " has made some changes to its promotions.";

    notifyCustomersOfChanges(
      business.businessId,
      NotificationChangeType.Promotions,
      notificationContents
    );
  }
  return true;
...
export const updateAppLoyaltyFromMerchant = async (
  ...
  try {
    ...
    if (loyaltyProgram.rewardTiers && loyaltyProgram.terminology) {
      var loyaltyRewardTier = await updateAppLoyaltyRewardTiersFromMerchant(
    ...
    if (loyaltyProgram.terminology) {
      var loyaltyPromotion = await updateAppLoyaltyPromotionsFromMerchant(
        promotions,
        loyaltyProgram.terminology,
        loyalty
      );
...
export const removeOldAccrualRules = async (
...
export const updateAppLoyaltyAccrualsFromMerchant = async (
  ...
  for (var loyaltyAccrualRule of loyaltyProgramAccrualRules) {
    ...
    if (shouldRemoveAccrualWhenItemIsMissiong && existingAppAccural?.id) {
      await deleteAccrual(existingAppAccural.id);
